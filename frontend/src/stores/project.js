...
export const useProjectStore = defineStore(
  ...
  () => {
    ...
    const reset = () => {
      ...
      projects.value = [];
      loading.value = false;
      performanceDrafts.value = {};
    };

    const setPerformanceDraft = (projectId, payload) => {
      performanceDrafts.value = {
        ...performanceDrafts.value,
        [String(projectId)]: payload,
      };
    };
    const consumePerformanceDraft = (projectId) => {
      const key = String(projectId);
      const draft = performanceDrafts.value[key];
      if (!draft) return null;
      const { [key]: _consumed, ...remaining } = performanceDrafts.value;
      performanceDrafts.value = remaining;
      return draft;
    };
