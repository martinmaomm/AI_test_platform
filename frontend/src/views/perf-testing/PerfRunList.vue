<template>
  <section class="performance-runs">
    <div class="toolbar">
      <el-button @click="router.push({ name: 'PerfPlans' })">返回计划</el-button
      ><el-button :loading="loading" @click="loadRuns">刷新</el-button>
    </div>
    <el-alert v-if="!canRead" type="warning" :closable="false" show-icon
      >你没有查看本项目执行记录的权限。</el-alert
    >
    <el-empty
      ...
      <el-table-column label="模式 / 验证" min-width="160"
        ><template #default="{ row }"
          >{{ row.mode === "validation" ? "单用户验证" : "正式压测"
            ...
            v-if="row.mode === 'validation'"
            size="small"
            :type="validationStatusType(row.validation_status)"
            >{{ validationStatusLabel(row.validation_status) }}</el-tag
          ></template
        ></el-table-column
      >
      ...
      <el-table-column label="操作" width="90" fixed="right"
        ><template #default="{ row }"
          ><el-button
            ...
            type="primary"
            @click="
              router.push({ name: 'PerfRunDetail', params: { runId: row.id } })
            "
            >详情</el-button
...
const canReport = computed(
  () => performanceExecutionPermissions(authStore.user, member.value).canReport,
);
...
async function loadRuns() {
  ...
  try {
    const [access, response] = await Promise.all([
      getProject(id),
      getPerformanceRuns(id),
    ]);
    if (requestEpoch !== epoch || String(projectId.value) !== String(id))
      return;
    ...
    if (requestEpoch === epoch)
      ElMessage.error(performanceErrorMessage(error, "加载执行记录失败"));
  } finally {
