<template>
  <div class="api-spec-detail-page">
    ...
    <div class="workflow-container">
      ...
      <div class="left-panel">
        ...
        <div class="api-tree">
          <!-- 空状态 -->
          <div
            v-if="!data.endpoints || data.endpoints.length === 0"
            class="empty-state"
          >
            <el-icon size="48">
              <InfoFilled />
          ...
          <div
            v-else
            v-for="(endpoints, tagName) in groupedEndpoints"
            :key="tagName"
            class="tree-group"
          >
            <div class="tree-group-header" @click="toggleTag(tagName)">
              <el-icon
                :class="
                  data.expandedTags.includes(tagName)
                    ? 'ArrowDown'
                    : 'ArrowRight'
                "
              ></el-icon>
            ...
            <div
              class="tree-group-content"
              v-show="data.expandedTags.includes(tagName)"
            >
              <div
        ...
        <div v-if="selectedEndpoint" class="endpoint-details">
          <div class="detail-header">
            ...
            <div class="header-actions">
              <el-button
                type="success"
                @click="viewEndpointDetail(selectedEndpoint)"
              >
                查看端点详情
              </el-button>
              <ActionHelpTooltip
              ...
              <div class="ai-loading-container">
                    ...
                    <svg viewBox="0 0 100 100" class="ai-brain-icon">
                      <defs>
                        ...
                        >
                          <stop
                            offset="0%"
                            style="stop-color: #409eff; stop-opacity: 1"
                          />
                          <stop
                            offset="50%"
                            style="stop-color: #66b1ff; stop-opacity: 1"
                          />
                          <stop
                            offset="100%"
                ...
                <div class="ai-loading-text">
                  <h3 class="ai-title">
                    <span
                      class="ai-text-char"
                      v-for="(char, index) in aiLoadingText"
                      :key="index"
            ...
            <div v-else-if="selectedEndpoint" class="test-cases-workspace">
              <div class="workspace-header">
                ...
                <div class="workspace-header-actions">
                  <el-button
                    ...
                    type="info"
                    size="small"
                    @click="loadEndpointTests(selectedEndpoint)"
              ...
              <div class="test-cases-list">
                ...
                >
                  <template #item="{ element: testCase }">
                    <div
                      ...
                      @click="showTestCaseDetail(testCase)"
                    >
                      ...
                      <div class="test-case-body">
                        <div class="test-case-header">
                          ...
                          <div class="test-case-actions">
                            <el-tag
                              :type="getTestTypeType(testCase.test_type)"
                              size="small"
                            >
                ...
                <div v-else class="empty-test-cases">
                  ...
                  <h4>暂无测试用例</h4>
                  <p>点击右上角按钮进入工作区，确认候选草稿后再保存测试用例</p>
                </div>
              </div>
            </div>
    ...
    <!-- 生成的测试用例结果 -->
    <el-card
      v-if="showGeneratedResults && generatedTestResults"
      class="generated-results-card"
    >
      <template #header>
        <div class="card-header">
          ...
          <el-button
            type="primary"
            size="small"
            @click="showGeneratedResults = false"
          >
            关闭
          </el-button>
        </div>
      ...
      <div class="generated-results-content">
        ...
        <div class="generation-stats">
          <el-row :gutter="20">
            <el-col :span="6">
              <div class="stat-item">
                <div class="stat-number">
                  {{ generatedTestResults.total_cases || 0 }}
                </div>
            ...
            <el-col :span="6">
              <div class="stat-item">
                <div class="stat-number">
                  {{ generatedTestResults.total_flows || 0 }}
                </div>
            ...
            <el-col :span="6">
              <div class="stat-item">
                <div class="stat-number">
                  {{ generatedTestResults.total_scripts || 0 }}
                </div>
            ...
            <el-col :span="6">
              <div class="stat-item">
                <div class="stat-number">
                  {{ generatedTestResults.generation_method || "unknown" }}
                </div>
        ...
        <div
          v-if="
            generatedTestResults.test_cases &&
            generatedTestResults.test_cases.length > 0
          "
          class="test-cases-section"
        >
          <h4>📋 生成的测试用例</h4>
          <el-table
            :data="generatedTestResults.test_cases"
            style="width: 100%"
            size="small"
          >
            ...
            <el-table-column label="操作" width="120" fixed="right">
              <template #default="scope">
                <el-button
                  type="primary"
                  size="small"
                  @click="viewGeneratedTestCase(scope.row)"
                >
                  查看详情
                </el-button>
              </template>
        ...
        <div
          v-if="
            generatedTestResults.stateful_flows &&
            generatedTestResults.stateful_flows.length > 0
          "
          class="flows-section"
        >
          <h4>🔄 业务流程</h4>
          <el-table
            :data="generatedTestResults.stateful_flows"
            style="width: 100%"
            size="small"
          >
            <el-table-column prop="id" label="ID" width="80" />
            ...
            <el-table-column label="操作" width="120" fixed="right">
              <template #default="scope">
                <el-button
                  type="primary"
                  size="small"
                  @click="viewGeneratedFlow(scope.row)"
                >
                  查看详情
                </el-button>
              </template>
        ...
        <div
          v-if="
            generatedTestResults.httprunner_scripts &&
            generatedTestResults.httprunner_scripts.length > 0
          "
          class="scripts-section"
        >
          <h4>📜 HttpRunner脚本</h4>
          <div
            v-for="(script, index) in generatedTestResults.httprunner_scripts"
            :key="index"
            class="script-item"
          >
    ...
    <el-dialog
      ...
      :title="
        selectedEndpoint
          ? `为端点 ${selectedEndpoint.method} ${selectedEndpoint.path} 生成测试用例`
          : 'AI生成测试用例配置'
      "
      width="600px"
    ...
    >
      ...
      >
        ...
        <el-form-item label="生成选项">
          <el-checkbox v-model="aiForm.include_assertions"
            >包含断言</el-checkbox
          >
          <el-checkbox v-model="aiForm.include_negative_cases"
            >包含负向测试</el-checkbox
          >
        </el-form-item>
      </el-form>
    ...
    <!-- 端点详情对话框 -->
    <el-dialog
      v-model="showEndpointDetailDialog"
      title="端点详情"
      width="800px"
      :close-on-click-modal="false"
    >
      <div v-if="selectedEndpoint" class="endpoint-detail">
        ...
        <el-card class="detail-section compact" shadow="never">
          ...
          <el-descriptions :column="3" border size="small">
            <el-descriptions-item label="HTTP方法">
              <el-tag
                :type="getMethodType(selectedEndpoint.method)"
                size="small"
              >
            ...
            <el-descriptions-item label="状态">
              <el-tag
                :type="selectedEndpoint.is_active ? 'success' : 'danger'"
                size="small"
              >
            ...
            <el-descriptions-item
              label="标签"
              :span="3"
              v-if="selectedEndpoint.tags && selectedEndpoint.tags.length > 0"
            >
        ...
        <!-- 请求头参数 -->
        <el-card
          class="detail-section compact"
          shadow="never"
          v-if="getHeaderParameters(selectedEndpoint).length > 0"
        >
          <template #header>
          ...
          <el-table
            :data="getHeaderParameters(selectedEndpoint)"
            style="width: 100%"
            size="small"
            :show-header="true"
          >
        ...
        <el-card
          class="detail-section compact"
          shadow="never"
          v-if="getPathParameters(selectedEndpoint).length > 0"
        >
          <template #header>
          ...
          <el-table
            :data="getPathParameters(selectedEndpoint)"
            style="width: 100%"
            size="small"
            :show-header="true"
          >
        ...
        <el-card
          class="detail-section compact"
          shadow="never"
          v-if="getQueryParameters(selectedEndpoint).length > 0"
        >
          <template #header>
          ...
          <el-table
            :data="getQueryParameters(selectedEndpoint)"
            style="width: 100%"
            size="small"
            :show-header="true"
          >
        ...
        <el-card
          ...
          v-if="
            ...
            Object.keys(selectedEndpoint.request_body).length > 0
          "
        >
          <template #header>
          ...
          >
            <el-descriptions-item label="必需">
              <el-tag
                :type="
                  selectedEndpoint.request_body.required ? 'danger' : 'info'
                "
                size="small"
              >
            ...
            <el-descriptions-item label="内容类型">
              <div v-if="selectedEndpoint.request_body.content">
                <el-tag
                  v-for="(content, contentType) in selectedEndpoint.request_body
                    .content"
                  :key="contentType"
                  size="small"
          ...
          <!-- 请求体字段列表 -->
          <div
            v-if="getRequestBodyFields(selectedEndpoint).length > 0"
          ...
          >
            ...
            <el-table
              :data="getRequestBodyFields(selectedEndpoint)"
              style="width: 100%"
              size="small"
              :show-header="true"
            >
              ...
              >
                <template #default="scope">
                  <div
                    v-if="
                      scope.row.constraints && scope.row.constraints.length > 0
                    "
                    style="font-size: 11px"
                  >
                    <span
                      v-for="(constraint, index) in scope.row.constraints"
                      :key="index"
        ...
        >
          ...
          <el-collapse v-model="activeResponseStatus" accordion>
            <el-collapse-item
              v-for="([statusCode, response], index) in Object.entries(
                selectedEndpoint.responses,
              )"
              :key="statusCode"
              :name="statusCode"
            >
              <template #title>
                <div class="response-header">
                  ...
                  >
                    <el-tag
                      v-for="contentType in Object.keys(response.content)"
                      :key="contentType"
                      size="small"
                      type="info"
              ...
              >
                <div
                  v-for="(contentData, contentType) in response.content"
                  :key="contentType"
                  class="content-type-section"
                >
                  ...
                  <div v-if="contentData.schema" class="schema-section">
                    ...
                    <div class="schema-viewer">
                      <pre class="json-pre">{{
                        formatSchema(contentData.schema)
                      }}</pre>
                    </div>
                  </div>
    ...
    <!-- 创建测试用例对话框 -->
    <el-dialog
      v-model="showCreateTestCaseDialog"
      title="创建测试用例"
      width="600px"
      :close-on-click-modal="false"
    >
      ...
      >
        ...
        <el-form-item label="测试端点" prop="endpoint_id">
          <el-select
            v-model="testCaseForm.endpoint_id"
            placeholder="选择测试端点"
          >
            <el-option
              v-for="endpoint in endpoints"
        ...
        <el-form-item label="测试类型" prop="test_type">
          <el-select
            v-model="testCaseForm.test_type"
            placeholder="选择测试类型"
          >
        ...
        <el-form-item label="测试数据">
          <el-input
            v-model="testCaseForm.test_data"
            type="textarea"
            :rows="5"
  ...
  test_type_configs: [
    {
      ...
      validator: (rule, value, callback) => {
        const enabledTypes = Object.values(value).filter(
        ...
        if (enabledTypes.length === 0) {
          ...
          // 检查启用的测试类型是否都设置了数量
          const invalidConfigs = enabledTypes.filter(
...
const editRules = {
  ...
  spec_type: [{ required: true, message: "请选择规范类型", trigger: "change" }],
};
...
const onTestTypeChange = (testType) => {
  if (aiForm.test_type_configs[testType].enabled) {
    // 如果启用，确保有默认数量
    if (
      !aiForm.test_type_configs[testType].count ||
      aiForm.test_type_configs[testType].count < 1
    ) {
...
const getTotalFilteredEndpoints = computed(() => {
  return Object.values(groupedEndpoints.value).reduce(
...
const selectEndpoint = async (endpoint) => {
  ...
  try {
    const response = await getEndpointTestCases(
    ...
    if (response?.success && response.data) {
      const raw =
        response.data.items || response.data.test_cases || response.data || [];
...
const showTestCaseDetail = async (testCase) => {
  try {
    ...
    const response = await getAPITestCase(
      projectStore.currentProjectId,
      testCase.id,
    );
  ...
  for (const [contentType, content] of Object.entries(
    ...
    if (content.schema && content.schema.properties) {
      ...
      for (const [fieldName, fieldSchema] of Object.entries(
        ...
        // 收集约束信息
        if (
          fieldSchema.min_length !== null &&
          fieldSchema.min_length !== undefined
        ) {
          constraints.push(`最小长度: ${fieldSchema.min_length}`);
        }
        if (
          fieldSchema.max_length !== null &&
          fieldSchema.max_length !== undefined
        ) {
...
// 生成测试
const generateTests = () => router.push("/api-testing/workspace");
...
const pollTaskStatus = async (taskId, endpoint = null) => {
  try {
    ...
    const poll = async () => {
      ...
      try {
        const response = await getEndpointTestGenerationStatus(
          projectStore.currentProjectId,
          taskId,
        );
        ...
        if (
          ...
          if (endpoint) {
            ...
            if (
              ...
              // 记录刷新前的已有用例数
              const beforeCount = endpointTests.value
                ? endpointTests.value.length
                : 0;
              ...
              if (result && (result.test_cases || result.items)) {
                ...
                if (Array.isArray(cases)) {
                  ...
                  const existingIds = new Set(
                    endpointTests.value.map((tc) => tc.id),
                  );
                  const toPrepend = newCases.filter(
                  ...
                  if (toPrepend.length > 0) {
                    endpointTests.value = [
                      ...toPrepend,
                      ...endpointTests.value.filter(
                        (tc) => !newCases.some((n) => n.id === tc.id),
                      ),
                    ];
                  } else {
                  ...
                  // 优先使用真实提取到的新用例数量
                  generatedCount =
                    toPrepend.length > 0 ? toPrepend.length : cases.length;
                } else {
                  // 如果返回的是数字或嵌套对象，重新拉取列表对比
                  await loadEndpointTests(endpoint);
                  const afterCount = endpointTests.value
                    ? endpointTests.value.length
                    : 0;
                ...
                if (generatedCount === 0) {
                  generatedCount =
                    taskData.total_cases || result?.total_cases || 0;
                }
              }
            ...
            ElMessage.success(
              `成功生成 ${stats.testCases} 个测试用例, ${stats.flows} 个业务流程, ${stats.scripts} 个HttpRunner脚本`,
            );
...
const runTestCase = async (testCase) => {
  try {
    const result = await executeAPITestCase(
      projectStore.currentProjectId,
      testCase.id,
    );
...
const duplicateTestCase = async (testCase) => {
  try {
    const detailRes = await getAPITestCase(
      projectStore.currentProjectId,
      testCase.id,
    );
  ...
  try {
    ...
    const payload = {
      ...
      script_content: JSON.stringify({
        config: {
          name: testCaseForm.name || "未命名",
          base_url: "",
          variables: {},
        },
        teststeps: [
          {
            name: testCaseForm.name || "步骤1",
