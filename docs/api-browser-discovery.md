...
- **自动允许跨域请求**：默认开启，自动发现接口来源时直接放行并记录跨域来源；关闭后对新跨域来源逐次确认。该选项随任务保存，重试继承原设置。
...
不知道 API origin 时留空即可。采集器在首次导航前启动，默认自动允许探索页面发起的跨域请求，无需逐次点击。关闭「自动允许跨域请求」后，新跨主机来源会在发送前暂停并展示地址、方法和路径；点击「允许」继续同一个浏览器的原请求，点击「拒绝」中止该请求，不重放登录或增删改查。等待确认不占用探索时限，但整轮累计等待最多 300 秒，超时后保留证据并关闭浏览器。明确填写 API origin 时仍按指定来源采集正文。性能项目的新探索默认 900 秒（服务器上限更低时遵循上限）；此前版本中未采集的登录正文仍无法事后恢复。
