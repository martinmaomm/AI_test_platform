...
改动最初在 `codex/validation-raw-details` 隔离工作树中完成。用户随后授权发布，已完成双架构公共发行、代码合入和本机平台切换；现有 0.3.1 节点由用户手动升级后再重新验证。发布结果、实际接口回读和远端边界见 [0.3.2 发布验收](2026-09-21-performance-node-0.3.2-release.md)。
...
后续公共发行已复用上述镜像和归档，完成匿名拉取、索引及平台入口验证；0.3.1 发行目录保留用于回退。升级远端节点时保留原身份卷，不重新注册。主工作区原有三个浏览器 ZIP 未提交。
