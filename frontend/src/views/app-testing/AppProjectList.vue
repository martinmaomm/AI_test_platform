<template>
  <div class="page-wrapper">
    <div class="main-content">
      <el-page-header content="App 测试项目列表" class="page-header">
        <template #icon>
      ...
      <div v-loading="loading" class="projects-content">
        <el-empty v-if="!loading && projectList.length === 0" description="暂无 App 测试项目" />
      ...
      <el-dialog v-model="showCreateDialog" title="新建 App 项目" width="500px" :close-on-click-modal="false" @close="resetCreateForm">
...
import { Plus, Cellphone } from '@element-plus/icons-vue'
...
// 加载项目列表（严格携带 project_type: 'app'）
const loadProjects = async () => {
  loading.value = true
  try {
    const response = await getProjects({ project_type: 'app' })
...
const enterProject = async (project) => {
  try {
    ...
    router.push('/app-testing/pom-parser')
  } catch {
