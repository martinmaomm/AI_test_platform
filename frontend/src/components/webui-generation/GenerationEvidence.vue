<template>
  <div v-if="trace && Object.keys(trace).length" class="trace-content">
    ...
    <div class="trace-section"><h5>最终路径定稿</h5><p>状态：{{ finalizationStatus }}</p><p v-if="trace.finalization?.message">{{ trace.finalization.message }}</p></div>
...
const finalizationStatus = computed(() => ({ valid: '已完成', invalid: '已失效或被拒绝', missing: '未定稿' })[trace.value?.finalization?.status] || '未定稿')
