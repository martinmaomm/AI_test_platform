...
平台管理员进入性能项目的“节点管理”，点击“添加节点”，填写名称和网络位置。创建后复制网页提供的**整条安装命令**，在已有 Docker 的 Linux 终端执行。命令依次拉取镜像、添加版本标签并启动容器，任一步失败就停止；需要 root 或 Docker 操作权限，页面不需要填写 SSH 密码。
...
- 拉取成功后，命令会把该固定镜像标记为同仓库的版本标签（例如 `martinmao9/performance-node:0.4.1`），便于在 `docker images` 中辨认。实际启动继续使用固定摘要，标签不会改变运行内容。请整条复制执行，不能只复制最后的 `docker run`，也不要仅在开头加 `sudo`；使用有 Docker 权限的终端。
...
2. 回网页勾选“我已停止并删除旧容器”，点击“生成新版安装命令”，将整条命令复制到原节点主机执行，自动拉取固定镜像、添加版本标签并启动。
...
发布工具验证各架构远端摘要、配置与匿名拉取后写入 `registry_ref`；`--publish-index` 将两种已验证的镜像组成固定索引，验证索引及其两个平台均可匿名拉取后，再发布简洁的版本标签（例如 `:0.4.1`）。版本标签必须指向完全相同的索引；已存在且一致则复用，不一致则拒绝覆盖。匿名回读通过后，manifest 记录固定的 `registry_index_ref` 和便于辨认的 `registry_version_ref`。网页安装命令仍以固定摘要拉取和启动，**不使用 `latest`，不因仓库失败自动降级或关闭 TLS**。平台公共 CA 只用于平台控制连接，不传给 Docker Hub。Docker Hub 在目标网络不可达或限流时仍可能下载失败，并不能保证所有网络都提速。

已发布的同版本固定镜像可以用相同的发布参数再次执行，幂等补齐版本标签；不会重新构建或覆盖镜像内容。已经运行的节点无需因标签变化而重装；远端本地标签只有执行打标命令或新版安装流程后才会出现。
