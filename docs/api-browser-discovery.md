...
没有 Swagger 时，可以在 **API 对话工作区 → 从网页探索生成** 中，让 AI 操作测试网站，平台记录真实接口，再在当前页面继续生成用例。
...
进入「从网页探索生成」，直接在页内表单填写以下信息，无需先新建空白 API 工作区：
...
探索期间可查看进度、工具次数、记录数、当前动作，也可以取消。刷新页面后从历史任务继续查看。切换标签或离开页面不会取消已提交的任务；未保存内容会提示确认。只有明确点击取消才请求停止对应任务。取消不会终止其他任务，也不会恢复原浏览器会话；需要继续时新建任务。
...
**交接只创建接口来源和工作区，不发送被测请求，不保存正式用例。** 原测试描述保存在需求历史，API 地址、模型、已选接口和生成提示自动带入工作区。页面保持「从网页探索生成」，显示只读的来源名称和任务标识，不需要再选择 API 文档。检查后再明确点击生成并验证；场景编辑、调试、修复、保存和导出的操作与文档来源一致。
