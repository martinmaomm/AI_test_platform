<template>
  <section v-if="hasRepairContent" class="repair-panel workspace-section">
    <div class="section-heading">
      <div><h5>AI 辅助调试</h5><p>原草稿保持不变。候选脚本仅在你二次确认采用后才写入工作区。</p></div>
    ...
    <section v-if="repairExecution || repairExecutionLoading" class="repair-execution">
      ...
      <WebUITestCaseExecutionDetail v-else-if="repairExecution" :execution="repairExecution" />
    </section>
  </section>
</template>
...
const repairLabel = computed(() => ({ pending: '等待 AI 分析', running: 'AI 正在修复', candidate_ready: '候选待审核', candidate_passed: '候选验证通过', failed: '修复未完成' })[repair.value.status] || '修复状态未知')
...
const phaseLabel = computed(() => ({ collecting: '收集失败证据', analyzing: '分析失败原因', exploring: '定向检查页面', validating: '验证候选脚本', completed: '处理完成' })[repair.value.phase] || '等待处理')
...
const executionStatusLabel = status => ({ passed: '执行通过', failed: '执行失败', error: '执行异常', incomplete: '验证未完成', not_run: '未执行浏览器验证', pending: '等待执行', running: '正在执行' })[status] || '等待执行'
const attemptFailure = attempt => attempt?.summary || attempt?.failure_summary || attempt?.error_message || attempt?.message || ''
const blockerText = blocker => typeof blocker === 'string' ? blocker : blocker?.message || '存在未说明的修复阻塞项。'
