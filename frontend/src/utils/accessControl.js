...
export const canEditManagedUser = (operator, target) => {
  ...
  return isSuperuser(operator) || (isPlatformAdmin(operator) && target.role !== 'admin' && !target.is_staff)
}

export const canAccessRoute = (user, route) =>
  !route.meta?.requiresPlatformAdmin || isPlatformAdmin(user)

export const visibleDashboardLayout = (layout, user) => {
  if (isPlatformAdmin(user)) return layout
  const restricted = new Set(['section-infra', 'portal-ai-config', 'portal-settings'])
  return layout.filter((item) => !restricted.has(item.i))
}
