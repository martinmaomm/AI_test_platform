...
没有 Swagger 时，可启用“从网页探索生成”：在页面内填写页面 URL 和测试目标，API origin 默认自动识别，也可在高级设置手动指定。AI 通过同一个 Playwright MCP 浏览器操作网站，平台从第一条业务请求开始记录真实网络交互；无法确定归属的跨主机接口须先确认，再继续原请求。检查并选择样本后，仍在网页探索页面继续 API 工作区的生成、调试和保存，不需要选择 Swagger。交接本身不执行 API、不保存正式用例。观察样本不是完整契约，也不保证覆盖所有接口。探索功能默认关闭，启用、使用与限制见 [网页探索发现 API](docs/api-browser-discovery.md)。
