<template>
  <section class="generation-card result-panel">
    ...
    <GenerationStopContext v-if="showStopContext" :generation="generation" :failure-reason="failureReason" />
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="脚本工作区" name="script"><el-alert v-if="draftConflict" type="warning" :closable="false" show-icon title="工作区已更新；你的本地编辑仍保留，刷新会丢弃这些未保存内容。"><template #default><el-button size="small" type="warning" plain @click="emit('discard-local-draft')">丢弃本地编辑并刷新</el-button></template></el-alert><GenerationWorkspace v-if="generation?.script_draft || draft?.script_draft" ref="workspaceRef" :generation="generation" :draft="draft" :busy="busy" :draft-saving="draftSaving" :debugging="debugging" :repairing="repairing" :debug-execution="debugExecution" :debug-execution-loading="debugExecutionLoading" @update-draft="emit('update-draft', $event)" @save-draft="emit('save-draft')" @debug="emit('debug', $event)" @repair="emit('repair', $event)"><template #repair><RepairPanel :repair="generation?.workspace?.repair" :busy="repairBusy" :applying="repairApplying" :discarding="repairDiscarding" :repair-execution="repairExecution" :repair-execution-loading="repairExecutionLoading" :selected-execution-id="selectedRepairExecutionId" @apply="emit('apply-repair', $event)" @discard="emit('discard-repair', $event)" @view-execution="emit('view-repair-execution', $event)" /></template></GenerationWorkspace><el-empty v-if="!generation?.script_draft && !draft?.script_draft" description="脚本草稿尚未生成" :image-size="70" /></el-tab-pane>
      ...
      <el-tab-pane label="质量报告" name="quality"><GenerationQualityReport :report="qualityReport" /></el-tab-pane>
...
import { canRetryScriptFromTrace, canSaveGeneratedDraft, formatGenerationLifecycleTime, generationDraftCompletion, generationFailureReason, generationLifecycleStateLabel, generationResolutionHint, generationStatusLabel, isPausedGeneration, modelInfoLabel, shouldShowGenerationStopContext } from '@/composables/webUIScriptGenerationPresentation'
...
import GenerationStopContext from './GenerationStopContext.vue'
...
const repairEntry = computed(() => {
  const status = props.generation?.workspace?.repair?.status
  if (!status || status === 'idle') return null
  const label = {
    pending: '等待 AI 分析', running: '正在修复', candidate_ready: '候选待审核',
    candidate_passed: '候选已通过实际验证', failed: '修复未完成'
  }[status] || '已有修复记录'
  return { label }
})
const openRepair = async () => {
  ...
  await nextTick()
  await workspaceRef.value?.revealRepair?.()
}
const qualityReport = computed(() => props.draft?.dirty
  ? { status: 'stale', message: '本地草稿已修改，上一版静态检查结果已失效。请保存草稿后查看本版结果。' }
  : props.generation?.quality_report)
...
const requestResumeExploration = async () => {
  ...
  try {
    const result = await ElMessageBox.prompt('请补充已知现场：已新增的数据标识、已完成的操作、尚未完成的操作；未检查时请先查询确认。无需填写 DOM。', '补充现场说明并继续', {
      inputPlaceholder: '例如：已新增的数据标识为……；已完成……；尚未完成……；未检查请先查询确认。',
    ...
    await ElMessageBox.confirm('将保留草稿和轨迹，并启动新的浏览器会话，可能需要重新登录。请先核对已有数据，再补充剩余操作；这不是从失败代码行断点续跑，不会直接重放旧脚本；智能体会按说明继续探索，请核对已有数据。', '确认恢复探索', {
...
<style scoped>
.generation-card { padding: 20px; background: var(--page-content-bg); border: 1px solid var(--app-border); border-radius: 10px; }.result-panel { min-width: 0; max-width: 100%; overflow: hidden; }.result-panel :deep(.el-tabs), .result-panel :deep(.el-tabs__content), .result-panel :deep(.el-tab-pane) { min-width: 0; max-width: 100%; }.result-header { display: flex; gap: 16px; align-items: flex-start; justify-content: space-between; margin-bottom: 14px; }.result-header h4 { margin: 0; color: var(--app-text-primary); font-size: 16px; }.result-header p, .technical-note { margin: 5px 0 0; color: var(--app-text-secondary); font-size: 13px; }.result-actions { display: flex; flex-wrap: wrap; gap: 8px; justify-content: flex-end; }.resume-action { display: inline-flex; }.resume-explanation { flex-basis: 100%; max-width: 620px; text-align: right; line-height: 1.6; }.repair-entry { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 14px; padding: 10px 12px; border: 1px solid var(--el-color-warning-light-5, #f3d19e); border-radius: 6px; background: var(--el-color-warning-light-9, #fdf6ec); color: var(--app-text-regular); font-size: 13px; }.resolution-hint { margin-bottom: 16px; }.technical-list { display: grid; grid-template-columns: 150px minmax(0, 1fr); gap: 9px 14px; margin: 0; color: var(--app-text-regular); font-size: 13px; }.technical-list dt { color: var(--app-text-secondary); }.technical-list dd { margin: 0; word-break: break-all; }.technical-output { max-height: 360px; overflow: auto; white-space: pre-wrap; overflow-wrap: anywhere; padding: 12px; background: var(--app-bg-secondary, #f5f7fa); border-radius: 6px; color: var(--app-text-regular); font-size: 12px; line-height: 1.6; } @media (max-width: 640px) { .result-header { flex-direction: column; }.result-actions { justify-content: flex-start; }.resume-explanation { text-align: left; }.repair-entry { align-items: flex-start; flex-direction: column; }.technical-list { grid-template-columns: 1fr; gap: 2px; }.technical-list dd { margin-bottom: 10px; } }
</style>
