...
当前主要能力是 **UI 自动化、API 自动化和项目知识库**。性能测试正在分批建设：已提供计划、压测目标及节点接入管理，尚不能从平台发起压测；安全测试仍是不可用的开发中入口。
...
| 性能测试（建设中） | 管理性能项目、受控目标、请求计划和节点注册/心跳；真实压测执行、指标与报告待下一批接入 |
...
当前仓库以本地和测试环境启动流程为主，没有整个平台的一键 Docker / Compose 部署方案。独立的 [性能节点客户端](performance-node/README.md) 提供 Docker / Compose 打包，仅支持节点登记和心跳。`npm run dev` 是开发服务，生产部署还需要构建前端、配置反向代理、进程管理、访问控制与备份。
