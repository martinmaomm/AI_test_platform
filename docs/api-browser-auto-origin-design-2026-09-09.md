...
- 2026-09-18 调整：确认等待不消耗探索预算，整轮累计等待独立限制为 300 秒；取消、关闭、任一预算超时必须释放挂起请求。已结束任务不能被确认接口重新启动。
...
2026-09-18 更新：新任务增加 `auto_approve_origins` 开关，默认 true，保存到任务 limits。开启后自动模式下页面发起的新跨域请求自动放行并记录已发现来源；关闭时保留逐次确认及独立等待预算。旧任务缺省为 false，重试继承原任务设置。明确指定 API origin 时不扩大正文采集范围，不改变浏览器 CORS 设置或模型导航目标范围。
