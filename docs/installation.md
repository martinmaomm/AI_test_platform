...
2. **MCP 配置**：添加上一节的固定 Playwright MCP JSON 并启用。预检要求 `@executeautomation/playwright-mcp-server` 使用明确版本、`npx` stdio 启动；不要添加端口模式，也不要把 token 写进日志或截图。
...
- 报告/通知：用有权限的账号打开报告链接，确认地址不是其他设备无法访问的 localhost；通知发送需另行确认接收对象。
