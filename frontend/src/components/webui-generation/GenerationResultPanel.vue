<template>
  <section class="generation-card result-panel">
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="技术信息" name="technical"><el-collapse><el-collapse-item title="查看安全技术信息"><dl class="technical-list"><dt>生成记录 ID</dt><dd>{{ generation?.id || '—' }}</dd><dt>任务 ID</dt><dd>{{ generation?.celery_task_id || '—' }}</dd><dt>状态 / 阶段 / 进度</dt><dd>{{ generation?.status || '—' }} / {{ generation?.current_stage || '—' }} / {{ generation?.progress || 0 }}%</dd><dt>模型</dt><dd>{{ modelText }}</dd><dt>探索工具统计</dt><dd>{{ toolStatsText }}</dd><dt v-if="generation?.error_code">错误码</dt><dd v-if="generation?.error_code">{{ generation.error_code }}</dd><dt v-if="generation?.exploration_snapshot?.termination_reason">轨迹终止标记</dt><dd v-if="generation?.exploration_snapshot?.termination_reason">{{ generation.exploration_snapshot.termination_reason }}</dd></dl></el-collapse-item></el-collapse></el-tab-pane>
    </el-tabs>
  </section>
</template>
