...
当前主要能力是 **UI 自动化、API 自动化和项目知识库**。性能测试支持从网页探索整理真实接口，编辑请求、变量和业务断言，再通过独立节点进行单用户验证和单节点 Locust 压测；需要先配置 HTTPS 节点入口和独立控制器。安全测试仍是不可用的开发中入口。
...
| 性能测试 | 网页探索采集接口、可编辑请求/变量/断言、单用户验证、单节点 Locust 压测、停止和基础指标 |
...
| [性能网页探索与断言验收](docs/verification/2026-09-18-performance-browser-discovery.md) | 真实接口导入、变量与断言、单用户验证、0.3.0 节点升级及已验证范围 |
