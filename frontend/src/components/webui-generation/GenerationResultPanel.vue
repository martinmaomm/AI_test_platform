<template>
  <section class="generation-card result-panel">
    <div class="result-header">
      ...
      <div class="result-actions"><el-button v-if="canSave" type="success" :loading="saving" :disabled="busy" @click="requestSave">{{ generation?.is_saved ? '再次保存更新' : '保存到测试用例' }}</el-button><el-button v-if="generation?.is_saved" type="primary" plain @click="emit('open-test-case')">进入用例管理</el-button></div>
    </div>
...
const emit = defineEmits(['resolve', 'cancel', 'save', 'open-test-case', 'update-draft', 'save-draft', 'debug', 'repair', 'discard-local-draft'])
