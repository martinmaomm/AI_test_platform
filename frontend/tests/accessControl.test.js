...
test("ordinary users cannot enter global configuration routes or manage project metadata", () => {
  assert.equal(
    canAccessRoute(user, { meta: { requiresPlatformAdmin: true } }),
    false,
  );
  assert.equal(canManageProjectMetadata(user), false);
  assert.equal(
    canAccessRoute(admin, { meta: { requiresPlatformAdmin: true } }),
    true,
  );
});
...
test("only superusers can modify administrators and no one modifies self or superusers", () => {
  ...
  assert.equal(
    canEditManagedUser(admin, { id: 4, role: "admin", is_staff: true }),
    false,
  );
  assert.equal(
    canEditManagedUser(superuser, { id: 4, role: "admin", is_staff: true }),
    true,
  );
  assert.equal(canEditManagedUser(superuser, superuser), false);
  assert.equal(
    canEditManagedUser(superuser, { id: 5, is_superuser: true }),
    false,
  );
});
