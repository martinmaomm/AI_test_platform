<template>
  <section class="generation-card timeline-card">
    ...
    <p class="lifecycle-summary">任务状态：{{ lifecycleStateText }}；最近心跳：{{ lifecycleHeartbeatText }}；最近检查点：{{ lifecycleCheckpointText }}<template v-if="generation?.lifecycle?.interrupted_at">；中断：{{ lifecycleInterruptedText }}</template></p>
...
import { buildGenerationTimeline, formatGenerationLifecycleTime, generationLifecycleStateLabel, generationStatusLabel, isPausedGeneration, modelInfoLabel } from '@/composables/webUIScriptGenerationPresentation'
...
<style scoped>
.generation-card { padding: 18px 20px; background: var(--page-content-bg); border: 1px solid var(--app-border); border-radius: 10px; }.timeline-header { display: flex; justify-content: space-between; gap: 16px; align-items: flex-start; margin-bottom: 14px; }.timeline-header h4 { margin: 0; color: var(--app-text-primary); font-size: 16px; }.timeline-header p, .lifecycle-summary { margin: 5px 0 0; color: var(--app-text-secondary); font-size: 13px; }.lifecycle-summary { line-height: 1.6; }.timeline-meta { display: flex; gap: 10px; align-items: center; color: var(--app-text-secondary); font-size: 12px; text-align: right; }.generation-steps { margin-top: 22px; } @media (max-width: 820px) { .generation-steps { overflow-x: auto; padding-bottom: 8px; min-width: 780px; }.timeline-card { overflow-x: auto; } }
</style>
