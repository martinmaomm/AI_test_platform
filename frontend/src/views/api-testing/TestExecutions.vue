<template>
  <div class="test-runs-container" v-if="selectedProject">
    ...
    <el-card class="table-card" shadow="hover">
      ...
      <el-table :data="filteredTestRuns" v-loading="loading" class="execution-table" :row-class-name="getRowClassName" @selection-change="handleSelectionChange">
    ...
    <el-dialog v-model="detailDialogVisible" title="执行详情" width="90%" :close-on-click-modal="false" :show-close="true"
      ...
      <!-- 套件执行详情 -->
      <APITestSuiteExecutionDetai v-if="selectedRun && selectedRun.exec_type === 'suite'" 
...
import { getAPITestExecutions, deleteAPITestExecution, getAPITestCaseExecutionDetail, getAPITestSuiteExecutionDetail, getAPITestStatistics } from '@/api/apiTesting'
...
const loadStatistics = async () => {
  try {
    const response = await getAPITestStatistics(projectStore.currentProjectId)
...
const batchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedRuns.value.length} 个执行记录吗？此操作不可恢复。`,
      '批量删除确认',
      {
    ...
    for (const run of selectedRuns.value) {
      try {
        const response = await deleteAPITestExecution(projectStore.currentProjectId, run.id)
        if (response.success) {
          deletedCount++
        }
      } catch (error) {
...
onMounted(async () => {
  ...
  await loadTestRuns() // 再加载分页数据
  // 若 URL 带 id 参数，自动打开该执行记录详情
  const targetId = route.query?.id
  if (targetId) {
    const row = testRuns.value.find(r => String(r.id) === String(targetId))
    if (row) {
      ...
      try {
        ...
        // 可能是 suite 类型或记录不存在，忽略
      }
    }
    router.replace({ path: route.path, query: {} }) // 清除 query 避免刷新时重复打开
  }
})
</script>
