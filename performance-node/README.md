...
这是独立于 automation-platform 后端、Redis 与平台 `.env` 的性能节点客户端，当前版本为 `0.3.0`，节点管理协议为 `2`，请求快照协议为 `2`，固定引擎版本为 `2.43.3`。
...
0.2.x 可以继续上报心跳，但不能执行新版请求快照。先确认节点没有活动测试，再替换镜像；平台不会自动停止或重装远程容器。
...
如果使用原生或旧 `enroll/run` 方式，沿用原平台地址、CA 配置和身份目录，仅升级程序/镜像。不要为升级而重新注册同一个节点。升级后在平台核对 Agent 版本为 0.3.0，先运行单用户验证再发压。回退只替换程序/镜像并保留卷；旧程序不能执行 v2 计划。
