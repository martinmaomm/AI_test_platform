...
def publish_stage_changed(generation, display_name: str) -> None:
    ...
    logger.info(
        'WebUI V2 generation stage: generation_id=%s stage=%s status=%s progress=%s',
        generation.pk,
    ...
    try:
        send_node_start_notification_helper(
            ...
            node_name=f'v2_{generation.current_stage}',
        ...
        logger.warning('WebUI V2 阶段事件发送失败: generation_id=%s', generation.pk, exc_info=True)


def publish_terminal(generation) -> None:
    """The single V2 source for safe completion, review, failure and cancel events."""
    task_id = generation.celery_task_id or str(generation.pk)
    ...
    try:
        ...
        logger.warning(
            'WebUI V2 终态事件去重缓存不可用，将降级发送: generation_id=%s',
            generation.pk,
        )
    ...
    def release_deduplication_key() -> None:
        ...
        try:
            ...
            logger.warning(
                'WebUI V2 终态事件去重缓存清理失败: generation_id=%s',
                generation.pk,
            )
    ...
    try:
        ...
        if not delivered:
            ...
            logger.warning('WebUI V2 终态事件未送达，将允许后续重试: generation_id=%s', generation.pk)
    except Exception:
        ...
        release_deduplication_key()
        logger.warning('WebUI V2 终态事件发送失败: generation_id=%s', generation.pk, exc_info=True)
