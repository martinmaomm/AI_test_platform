...
def _generation_messages(*, conversation: list[dict[str, Any]], draft: dict[str, Any],
    ...
    rules = [
        ...
        '运行时通用变量每次执行只生成一次，同一轮所有步骤和变量别名复用同一个值；下一次独立执行才重新生成。两个业务变量若使用相同模板，即使变量名不同，值也相同。需要同一轮不同的数据时使用不同阶段前缀或后缀，例如 {"before_value":"before_${uuid4}","after_value":"after_${uuid4}"}；必须遵守用户前缀和文档字段格式约束，不能把重复引用 ${uuid4} 或 ${timestamp_ns} 当作再次调用函数。',
        ...
        '用户目标要求实际状态变化时，按目标语义与接口契约选择要改变的字段，不靠接口路径、字段名或操作名称的固定词表判断。变化前后输入必须不同；用文档支持的查询读取同一对象，提取变化前的实际字段值到独立变量（例如 observed_before），后续不得覆盖它。变化后再次精确查询同一对象，对该字段同时保留 eq 预期新值和 ne ${observed_before} 两条断言；仅 HTTP/业务成功、字段非空或 eq 一个与旧值相同的变量，不能证明变化。查询方式与字段必须来自文档或本轮证据，不得杜撰路径。',
        '上述状态变化要求不适用于明确验证幂等、重复提交或保持原值的场景；这些场景按用户预期验证未变化，不要强制制造新值。若缺少可靠的前后查询能力，在 summary 说明无法验证状态变化，不能声称已证明业务变化。',
