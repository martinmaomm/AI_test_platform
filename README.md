...
没有 Swagger 时，可启用“从网页探索生成”：在页面内填写页面 URL、业务 API origin 和测试目标，AI 通过同一个 Playwright MCP 浏览器操作网站，平台持续记录真实网络交互。检查并选择样本后，仍在网页探索页面继续 API 工作区的生成、调试和保存，不需要选择 Swagger。交接本身不执行 API、不保存正式用例。观察样本不是完整契约，也不保证覆盖所有接口。探索功能默认关闭，启用、使用与限制见 [网页探索发现 API](docs/api-browser-discovery.md)。
