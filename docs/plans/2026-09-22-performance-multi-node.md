...
日期：2026-09-22。状态：规划稿，待用户评审后实施。

用户已确认按 **2–5 个节点、总并发最多 1000 个虚拟用户** 规划。继续支持单节点。本次只提交设计文档，不改执行代码、业务数据库或线上服务，不发布节点，不发起压测。1000 是拟支持的配置规模，不是已测得的容量或 RPS 保证。

本方案细化 [原分布式性能测试设计](2026-09-15-distributed-performance-testing.md) 的多节点阶段，保留已有平台、独立控制器、Locust Master、节点 Agent/Worker 架构。现有运行版本为 Agent 0.3.2、协议 2、快照 2、Locust 2.43.3；这些是当前代码基线，不代表本方案已实现。
...
失联处理受心跳检测和命令租约影响，不能保证断网瞬间全员停止。准备、在途请求收尾、统计等待、进程强制回收和运行总时限采用以下预算；这些是首期建议值，需通过故障测试核定。
...
| 进程回收确认 | 30 秒 | 各节点并行确认，不按节点逐个累加；覆盖租约和强制终止余量 |
