...
新增能力需要节点 `0.3.0`（管理协议仍为 2，请求快照为 2）。旧 `0.2.x` 节点可以继续上报心跳，但不能执行新版计划。平台不会自动停止或重装已有节点，请在节点空闲时按[节点升级说明](../performance-node/README.md)更换镜像，保留身份卷。升级平台时的迁移将原 `expected_status` 转成状态码断言，不删除现有计划及历史记录。
