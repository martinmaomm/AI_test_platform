...
1. 生成候选并检查差异，点击“采用候选”更新可视化草稿；不会自动覆盖测试用例。
2. 编辑请求、变量、提取和断言，再明确点击调试。失败后可让 AI 根据本次请求/响应生成修复候选，不会自动重发业务请求。
3. 检查调试结果并保存用例。用例和套件使用同一 requests 执行核心；套件内按顺序执行，各用例独立 Session，不互相泄漏提取值或 Cookie。
...
实施与验收边界见 [API B 方案](docs/plans/2026-09-06-api-workspace-b.md)。
