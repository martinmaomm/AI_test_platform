...
2. **MCP 配置**：添加上一节的固定 Playwright MCP JSON 并启用。保存后点击“刷新工具”，确认显示“已发现 X 个工具”；“尚未检测”不代表零工具，检测失败会显示原因及检测时间。刷新只读取工具清单，不执行浏览器操作，也不改变启用状态。预检要求 `@executeautomation/playwright-mcp-server` 使用明确版本、`npx` stdio 启动；不要添加端口模式，也不要把 token 写进日志或截图。允许添加多条配置，但当前网页探索只选最新启用且结构符合要求的 Playwright 配置，不会自动轮询。
