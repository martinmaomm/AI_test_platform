<template>
  <div class="perf-workspace">
    ...
    <PerformancePlanEditor :visible="planDialog.visible" :item="planDialog.item" :targets="targets" :targets-loading="loading.targets" :target-error="targetLoadError" :limits="config.limits" :saving="saving.plan" @refresh-targets="refreshEditorTargets" @request-close="requestPlanClose" @closed="resetPlan" @save="savePlan" />
    ...
    <el-dialog v-model="installationDialog.visible" title="节点安装向导" width="680px" :close-on-click-modal="false" @closed="clearInstallationGuide">
      <template v-if="installationNode">
        <el-alert :type="installationStage.key === 'online' ? 'success' : ['offline', 'revoked'].includes(installationStage.key) ? 'warning' : 'info'" :closable="false" show-icon>{{ installationStage.text }}</el-alert>
        <el-descriptions :column="1" border class="installation-summary"><el-descriptions-item label="节点">{{ installationNode.name }}</el-descriptions-item><el-descriptions-item label="归属项目">{{ projectStore.currentProject?.name || detail?.name || '-' }}</el-descriptions-item><el-descriptions-item label="状态">{{ performanceNodeStatusLabel(installationNode.status) }}</el-descriptions-item><el-descriptions-item label="平台地址">{{ installationInfo?.platform_url || '-' }}</el-descriptions-item><el-descriptions-item label="支持架构">{{ installationArchitectureText(installationInfo?.supported_architectures) }}</el-descriptions-item></el-descriptions>
        <el-alert class="installation-requirements" type="info" :closable="false">需要 Linux 主机、Docker，以及 root 或 Docker 操作权限。新节点安装使用一条单行 docker run 命令，Docker 会按主机架构选择固定摘要镜像；节点主动连接平台，无需开放节点入站端口。</el-alert>
        <ul v-if="installationInfo?.requirements?.length" class="installation-requirement-list"><li v-for="requirement in installationInfo.requirements" :key="requirement">{{ requirement }}</li></ul>
        <el-alert v-if="installationInfo?.available === false" type="warning" :closable="false">{{ installationInfo.reason || '当前无法生成安装命令，请检查平台安装配置。' }}</el-alert>
        <template v-if="canUseInstallationCommand(installationNode, installationDialog.installation, installationCommandIsExpired)">
          <p class="expiry">凭证 {{ formatTime(installationDialog.installation.expires_at || installationDialog.expiresAt) }} 前有效且敏感，会出现在终端历史和 docker inspect 中；请只在目标节点执行。长期身份保存在独立 Docker volume 中。</p>
          <p class="expiry">注册异常可执行 <code>docker logs {{ installationDialog.installation.container_name }}</code> 查看；是否在线以平台收到真实 heartbeat 为准。</p>
          <el-input :model-value="installationDialog.installation.command" readonly type="textarea" :rows="4" />
          <el-button class="copy-command" type="primary" @click="copyInstallationCommand">复制安装命令</el-button>
        </template>
        <template v-else-if="installationDialog.loaded && installationAvailable && canRegenerateInstallation(installationNode, installationDialog.installation, installationCommandIsExpired)">
          <el-alert type="warning" :closable="false"><template v-if="installationCommandIsExpired">安装命令已过期。若此前已执行过命令，且确认节点仍未注册、同名失败容器存在，可先执行 <code>docker rm -f {{ installationInfo?.container_name }}</code> 移除该容器（不要删除身份 volume）；未执行过命令可直接重新生成。</template><template v-else>安装命令未保存在网页中。安装未完成可重新生成命令重试，节点不会自动作废。</template></el-alert>
          <el-button type="warning" :loading="saving.node" @click="regenerateInstallation(installationNode)">重新生成安装命令</el-button>
        </template>
        <template v-else-if="['offline', 'registered'].includes(installationStage.key)">
          <el-alert v-if="installationInfo?.container_name" type="warning" :closable="false">节点已安装，请执行 <code>docker restart {{ installationInfo.container_name }}</code> 并用 <code>docker logs {{ installationInfo.container_name }}</code> 检查注册或心跳；不要重复执行 docker run。长期身份保存在原 Docker volume 中，身份数据丢失请吊销后新建。</el-alert>
          <el-alert v-else type="warning" :closable="false">这是旧版已注册节点，请按原安装器创建的实际容器名执行 docker restart 和 docker logs；不要按新版命名猜测或重复执行 docker run。</el-alert>
        </template>
      </template>
...
const installationDialog = reactive({ visible: false, node: null, installation: null, expiresAt: '', loaded: false }); const nodeFormRef = ref(); const targetFormRef = ref()
...
async function loadList(kind, requestProjectId, requestEpoch, { silent = false } = {}) {
  const requestId = ++listRequestIds[kind]
  const isCurrent = () => active && requestId === listRequestIds[kind]
    && requestEpoch === epoch && String(projectId.value) === String(requestProjectId)
  loading[kind] = true
  const call = { plans: getPerformancePlans, nodes: getPerformanceNodes, targets: getPerformanceTargets }[kind]
  try {
    ...
    if (!isCurrent()) return
    const label = { plans: '压测计划', nodes: '节点', targets: '压测目标' }[kind]
...
async function refreshAll({ preservePlanDraft = false } = {}) { const requestProjectId = projectId.value; const requestEpoch = ++epoch; clearInterval(pollTimer); if (!requestProjectId || !active) return; if (projectStore.currentProject?.project_type !== 'perf') { ElMessage.warning('请先从性能测试项目列表选择性能项目'); router.replace('/perf-testing/projects'); return }; await Promise.all([loadAccess(requestProjectId, requestEpoch), loadConfig(requestProjectId, requestEpoch), loadList('targets', requestProjectId, requestEpoch), preservePlanDraft ? Promise.resolve() : loadList('plans', requestProjectId, requestEpoch), loadList('nodes', requestProjectId, requestEpoch)]); if (active && requestEpoch === epoch && String(projectId.value) === String(requestProjectId)) pollTimer = window.setInterval(() => { installationClock.value = Date.now(); return Promise.all([loadConfig(requestProjectId, requestEpoch), loadList('nodes', requestProjectId, requestEpoch, { silent: true })]) }, Math.max(5000, Math.min(10000, config.heartbeat_interval_seconds * 1000))) }
async function refreshEditorTargets() { const scope = captureScope(); if (!active || !scope.projectId || !planDialog.visible) return; await loadList('targets', scope.projectId, scope.epoch); }
function resetPlan() { planDialog.item = null }
...
function openPlan(item) { planDialog.item = item || null; planDialog.visible = true }
...
async function savePlan(payload) {
  ...
  if (!active || !planDialog.visible || loading.targets || targetLoadError.value
    || !targets.value.some((target) => String(target.id) === String(payload.target_id))) return
  const scope = captureScope()
  if (!scopeIsCurrent(scope)) return
  saving.plan = true
  try {
...
watch(projectId, () => { ++epoch; invalidateProjectUi(); if (active) refreshAll() })
watch(() => route.name, () => { if (active) syncActiveTab() })
const activate = () => {
  active = true
  syncActiveTab()
  refreshAll({ preservePlanDraft: planDialog.visible })
}
const deactivate = () => {
  active = false
  ++epoch
...
onBeforeUnmount(() => { deactivate(); invalidateProjectUi() })
</script>

<style scoped>
.perf-workspace { max-width: 1280px; margin: 0 auto; padding: 4px 10px 28px; }.execution-unavailable { margin-bottom: 18px; }.toolbar { min-height: 44px; display: flex; justify-content: space-between; align-items: center; gap: 16px; color: var(--app-text-muted); margin-bottom: 12px; }.method-tag { margin: 2px; }.installation-summary, .installation-requirements { margin-top: 16px; }.installation-requirement-list { margin: 10px 0; padding-left: 20px; color: var(--app-text-muted); }.copy-command { margin-top: 12px; }.expiry, .empty-node { color: var(--app-text-muted); font-size: 13px; }.run-summary { margin-top: 16px; } @media (max-width: 760px) { .toolbar { flex-direction: column; align-items: flex-start; } }
</style>
