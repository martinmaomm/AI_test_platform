...
管理器与服务的身份由 PID、内核原始启动标记、本次系统启动标识及完整命令/工作目录共同核验，系统时间校准不会改变归属。2026-09-21 起状态格式为 v2；升级前应在无活动任务时用旧版本停止服务和管理器，再启动新版本。v1 的普通启动时间不能安全转换为新身份，因此不会据此自动接管旧进程。若旧版本已显示“未纳管”，需逐项核对实际进程后优雅退出，再通过统一入口启动；不要删除状态文件后盲目重启或按进程名批量终止。该实现使用已固定的 psutil 7.2.2 内部身份接口，升级此依赖时也应完成受控重启及身份回归。
...
用户菜单中的“登录记录”只显示当前账号的成功登录时间、IP 和浏览器信息，管理员也不能通过此入口查看他人记录。新增表通过常规 `python manage.py migrate` 创建；历史登录不补录，页面刷新和 Token 自动续期不增加记录。IP 未能取得时显示“未知”，不查询地理位置。

默认 `LOGIN_RECORD_TRUSTED_PROXIES=127.0.0.1/32,::1/128`，只信任本机代理。`run_asgi.py` 已关闭 Uvicorn 自动转发头解析，若自行运行 Uvicorn，请加 `--no-proxy-headers`，由 Django 的可信代理逻辑统一判断来源。Vite 开发代理会覆盖外部传入的 IP 头，仅转发实际 TCP 对端 IP。

生产反向代理不在本机时，将它连接后端的实际 IP/CIDR 加入该配置，不要使用 `*`、`0.0.0.0/0`、`::/0` 或不必要的整片内网。代理需正确覆盖/追加 `X-Forwarded-For`，后端从右向左检查可信链。如果 Vite 前面还有外层代理，记录的是 Vite 实际看到的代理地址；正式部署应由受控反向代理直接代理后端 API，不能简单信任浏览器送来的 IP。NAT 地址不等于终端真实公网地址。

修改可信代理配置后重启后端；修改 Vite 代理后重载开发服务。记录写入失败只保留不含登录凭据的错误日志，不阻断已验证成功的登录。正式应用前还需另行确定记录保留周期，当前不会自动删除历史。
...
迁移前确认没有未结束的性能测试，停止后端、Celery 和已运行的性能执行控制器；迁移后按第 7 节和 13.1 节重新启动这三个服务。节点管理的后续迁移移除未使用的自定义标签并增加软删除标记；如需留存原标签，请在升级前备份。历史节点行、执行记录和报告不会删除。首页进入性能项目后，可以管理计划；平台管理员管理压测目标和节点，普通用户按项目权限访问。执行需要项目执行权限，执行详情需要报告权限。节点在线只代表心跳成功；运行进入“准备中”后还要验证加密连接、固定脚本版本和 Worker 身份，才实际发压。
...
注册凭证只显示一次。尚未注册时，命令丢失或过期可在安装指导中重新生成，旧命令立即失效；已经注册的节点不再支持重置身份。离线时先检查原容器与网络，长期身份丢失则吊销旧节点并新建。吊销不可恢复；有未结束任务时须明确确认请求停止，收尾可能导致报告不完整。已吊销且所有任务结束后才能删除节点，删除仅移出节点列表，保留历史执行记录和报告，不会卸载远程容器。
...
数据库临时不可达、连接丢失或调度租约过期时，控制器会暂停调度、停止本地执行进程，并在同一进程内自动尝试恢复，重试间隔为 1、2、4、8、16 秒，之后最多每 30 秒一次。控制器进程的 MySQL 连接、读、写超时各不超过 5 秒。数据库恢复后，须先等待旧执行进程退出并重新取得全局租约；已开始的测试会按执行不完整收尾，不自动重跑。尚未开始的排队任务在安全收尾后继续正常调度。数据库账号错误、SQL 或表结构错误仍会明确失败，不无限重试。

该修复仅涉及平台端，节点继续使用 0.3.1。更新代码后对已退出或运行旧代码的控制器执行一次 `./platform restart controller`；之后数据库临时断连无需人工重启。服务管理器的进程身份问题仍单独跟踪，详见 [数据库断线恢复验收](verification/2026-09-21-performance-controller-db-recovery.md)。

### 13.2 执行及限制

按“批准压测目标 → 编写请求计划（或网页探索导入）→ 接入节点 → 单用户验证 → 正式压测 → 查看执行记录”使用。当前每轮恰好一个节点，多个运行全局排队；虚拟用户之间并发，各用户内部步骤顺序执行。不支持任意 Python 脚本、多节点负载拆分或 API 场景自动转换。

网页探索复用固定版本 Playwright MCP 和系统中启用的模型，配置要求与 API 网页探索相同；不需要额外创建 API 项目。选择真实接口样本和已批准目标后，只生成未保存的计划草稿，不自动保存或发压。采样是有限证据，不是完整 Swagger：请核对脱敏值占位变量、业务断言和每轮数据策略。

- **准备步骤**：每个虚拟用户启动时执行一次，适合登录并提取令牌；每个用户隔离 Cookie 和变量。准备失败时该用户停止。
- **主流程**：循环执行。可用固定变量、准备步骤提取值、每轮唯一变量和本轮前序提取值。断言/提取失败中止本轮，不继续执行依赖步骤。
- **参数**：Query、Headers、JSON/Form/Raw 请求体分别编辑；完整 `${name}` 引用保留原类型。每轮唯一变量使用运行及虚拟用户唯一分量，不重放采样时的固定时间戳。暂不支持文件上传。
- **断言**：每步至少一条。支持 HTTP 状态、JSON 字段、列表长度、文本和响应头等检查。例如 `body.data` + `length_gt` + `0`；`200` 是数值，`"200"` 是字符串。一个请求的多条断言失败只计一次失败请求。
- **单用户验证**：固定一个用户执行一轮，使用与正式压测相同的请求和断言执行器。只有全部步骤真正执行成功才算通过；计划、目标或所选节点变化后，需重新验证，不复用陈旧结论。验证也会向测试目标发请求并可能修改数据。

单用户验证详情会按步骤显示成功、失败、跳过、请求 URL、响应正文、每条断言的期望/实际值及提取结果；敏感字段脱敏，过长内容标注截断。明细仅供具有报告权限的用户查看；历史未采集的响应无法补回。

当前执行模板需要节点 `0.3.1`（管理协议仍为 2，请求快照为 2）。旧 `0.2.x` / `0.3.0` 节点可以继续上报心跳，但创建运行时会提示升级。源码更新后仍需构建发行新版镜像并配置匹配的发行清单，不能直接使用旧 0.3.0 镜像接收新模板。平台不会自动停止或重装已有节点，请在节点空闲时按[节点升级说明](../performance-node/README.md)更换镜像，保留身份卷，再重新进行单用户验证。没有修改现有计划和历史记录的数据结构。
